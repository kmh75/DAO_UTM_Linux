...
QString autoCalibrationStateName(int state)
{switch(state){case UTM_COMPLIANCE_CAL_IDLE:return "IDLE";case UTM_COMPLIANCE_CAL_VALIDATING:return "VALIDATING";case UTM_COMPLIANCE_CAL_ZEROING_FORCE:return "ZEROING FORCE";case UTM_COMPLIANCE_CAL_CAPTURING_REFERENCE:return "CAPTURING REFERENCE";case UTM_COMPLIANCE_CAL_PRECHECK_APPROACH:return "PRE-CHECK APPROACH";case UTM_COMPLIANCE_CAL_PRECHECK_STABILIZING:return "PRE-CHECK STABILIZING";case UTM_COMPLIANCE_CAL_STOPPING_PRECHECK:return "STOPPING PRE-CHECK";case UTM_COMPLIANCE_CAL_WAITING_FULL_START:return "PRE-CHECK PASSED — USER CONFIRMATION REQUIRED";case UTM_COMPLIANCE_CAL_APPROACHING_TARGET:return "APPROACHING TARGET";case UTM_COMPLIANCE_CAL_FINE_APPROACH:return "FINE APPROACH";case UTM_COMPLIANCE_CAL_STABILIZING:return "STABILIZING";case UTM_COMPLIANCE_CAL_CAPTURING_POINT:return "CAPTURING POINT";case UTM_COMPLIANCE_CAL_NEXT_POINT:return "NEXT POINT";case UTM_COMPLIANCE_CAL_RELEASING_FORCE:return "RELEASING FORCE";case UTM_COMPLIANCE_CAL_STOPPING_RELEASE:return "STOPPING RELEASE";case UTM_COMPLIANCE_CAL_RETURNING:return "RETURNING";case UTM_COMPLIANCE_CAL_COMPLETE_PENDING_SAVE:return "COMPLETE — PENDING SAVE";case UTM_COMPLIANCE_CAL_ABORTING:return "ABORTING";case UTM_COMPLIANCE_CAL_ABORTED:return "ABORTED";case UTM_COMPLIANCE_CAL_FAULTED:return "FAULTED";case UTM_COMPLIANCE_CAL_PRECHECK_EXPIRED:return "PRE-CHECK EXPIRED";}return "UNKNOWN";}
...
{
    networkBackend_=std::make_unique<NmcliNetworkBackend>();networkService_=std::make_unique<NetworkService>(*networkBackend_);
    powerService_=std::make_unique<SystemPowerService>();
    shutdownCoordinator_=std::make_unique<ApplicationShutdownCoordinator>([this]{return controller_->shutdownActivity();},[this](QString& e){return controller_->orderlyShutdown(e);},[this](QString& e){return powerService_->RequestPowerOff(e);});
    ...
    complianceMainStatus_=new QLabel("COMPLIANCE C: OFF / T: OFF");
    for(auto* chip:{ethercatState_,servoState_,safetyState_,upperLimitState_,lowerLimitState_,externalStopState_,complianceMainStatus_}){chip->setObjectName("statusChip");chip->setMinimumWidth(105);chip->setAlignment(Qt::AlignCenter);statusRow->addWidget(chip);}
    ...
    commandMessage_ = new QLabel(controller_->isOffline()
        ? "Ready — offline mode" : "Ready");
    commandMessage_->setObjectName("messageBar");statusBar()->addWidget(commandMessage_,1);
    statusBar()->setSizeGripEnabled(false);statusBar()->setObjectName("MainStatusBar");
    ...
    connect(controller_,&UtmUiController::profileStatus,this,[this](const QString& m){commandMessage_->setText(m);commandMessage_->setProperty("error",false);});
...
QWidget* MainWindow::buildDashboard()
{
    ...
    auto* exitArea=new QFrame;exitArea->setObjectName("MainExitControls");auto* exitLayout=new QHBoxLayout(exitArea);exitLayout->setContentsMargins(8,4,8,4);exitLayout->addStretch();powerOffAfterExit_=new QCheckBox("Power off PC after exit");powerOffAfterExit_->setObjectName("MainPowerOffAfterExit");powerOffAfterExit_->setChecked(false);powerOffAfterExit_->setProperty("nonPersistentSystemField",true);exitButton_=button("EXIT","danger");exitButton_->setObjectName("MainExitButton");exitButton_->setMinimumWidth(120);exitLayout->addWidget(powerOffAfterExit_);exitLayout->addWidget(exitButton_);layout->addWidget(exitArea);connect(exitButton_,&QPushButton::clicked,this,[this]{requestOrderlyExit(powerOffAfterExit_->isChecked());});
    return page;
}
...
QWidget* MainWindow::buildSetup()
{
    ...
    auto* network=new QGroupBox("Network Settings");network->setObjectName("NetworkSettings");auto* nl=new QHBoxLayout(network);
    auto* ethernet=new QGroupBox("Ethernet");auto* el=new QFormLayout(ethernet);
    wiredAdapter_=new QComboBox;wiredMode_=new QComboBox;wiredMode_->addItems({"Automatic (DHCP)","Manual"});wiredIp_=new QLineEdit;wiredPrefix_=integer(24,0,32);wiredGateway_=new QLineEdit;wiredDns_=new QLineEdit;wiredStatus_=new QLabel("Not refreshed");wiredStatus_->setWordWrap(true);
    for(auto* w:{static_cast<QWidget*>(wiredAdapter_),static_cast<QWidget*>(wiredMode_),static_cast<QWidget*>(wiredPrefix_)})w->setProperty("nonPersistentSystemField",true);
    wiredAdapter_->setObjectName("NetworkEthernetAdapter");wiredMode_->setObjectName("NetworkIpMode");wiredStatus_->setObjectName("NetworkEthernetStatus");
    auto* networkRefresh=button("REFRESH");auto* wiredApply=button("APPLY","warning");wiredApply->setObjectName("NetworkEthernetApply");
    el->addRow("Adapter",wiredAdapter_);el->addRow("Status",wiredStatus_);el->addRow("IP Mode",wiredMode_);
    wiredManualFields_=new QWidget;wiredManualFields_->setObjectName("NetworkManualFields");auto* manual=new QFormLayout(wiredManualFields_);manual->setContentsMargins(0,0,0,0);manual->addRow("IP Address",wiredIp_);manual->addRow("Subnet Prefix",wiredPrefix_);manual->addRow("Gateway",wiredGateway_);manual->addRow("DNS",wiredDns_);el->addRow(wiredManualFields_);auto* ethernetActions=new QHBoxLayout;ethernetActions->addWidget(wiredApply);ethernetActions->addWidget(networkRefresh);el->addRow(ethernetActions);nl->addWidget(ethernet,1);
    auto* wifi=new QGroupBox("Wi-Fi");auto* wl=new QVBoxLayout(wifi);auto* wifiDetails=new QFormLayout;
    wifiAdapter_=new QComboBox;wifiStatus_=new QLabel("No Wi-Fi adapter available.");wifiStatus_->setWordWrap(true);wifiNetworks_=new QTableWidget(0,3);wifiNetworks_->setHorizontalHeaderLabels({"SSID","Signal","Security"});wifiNetworks_->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);wifiNetworks_->setSelectionBehavior(QAbstractItemView::SelectRows);wifiNetworks_->setMinimumHeight(150);wifiPassword_=new QLineEdit;wifiPassword_->setEchoMode(QLineEdit::Password);wifiPassword_->setPlaceholderText("Network password");wifiScan_=button("SCAN");wifiOn_=button("TURN ON");wifiOff_=button("TURN OFF");wifiConnect_=button("CONNECT");wifiDisconnect_=button("DISCONNECT");
    wifiAdapter_->setProperty("nonPersistentSystemField",true);
    wifiAdapter_->setObjectName("NetworkWifiAdapter");wifiStatus_->setObjectName("NetworkWifiStatus");wifiOn_->setObjectName("NetworkWifiOn");wifiOff_->setObjectName("NetworkWifiOff");wifiScan_->setObjectName("NetworkWifiScan");wifiConnect_->setObjectName("NetworkWifiConnect");wifiDisconnect_->setObjectName("NetworkWifiDisconnect");
    for(auto* control:{wifiOn_,wifiOff_,wifiScan_,wifiConnect_,wifiDisconnect_})control->setEnabled(false);wifiPassword_->setEnabled(false);wifiNetworks_->setEnabled(false);
    wifiDetails->addRow("Adapter",wifiAdapter_);wifiDetails->addRow("Status",wifiStatus_);wl->addLayout(wifiDetails);wl->addWidget(new QLabel("Available Networks"));wl->addWidget(wifiNetworks_);auto* passwordRow=new QFormLayout;passwordRow->addRow("Password",wifiPassword_);wl->addLayout(passwordRow);auto* wb=new QHBoxLayout;for(auto* b:{wifiOn_,wifiOff_,wifiScan_,wifiConnect_,wifiDisconnect_})wb->addWidget(b);wl->addLayout(wb);nl->addWidget(wifi,1);layout->addWidget(network,3,0,1,2);
    auto* ethercatInfo=new QLabel(QString("EtherCAT Interface: %1 (Managed by UTM)").arg(controller_->profile().adapterName.isEmpty()?"Not selected":controller_->profile().adapterName));ethercatInfo->setObjectName("EthercatNetworkInfo");layout->addWidget(ethercatInfo,4,0,1,2);
    connect(wiredMode_,qOverload<int>(&QComboBox::currentIndexChanged),this,[this](int index){wiredManualFields_->setEnabled(index==1);wiredManualFields_->setVisible(index==1);});wiredManualFields_->setEnabled(false);wiredManualFields_->setVisible(false);
    connect(networkRefresh,&QPushButton::clicked,this,&MainWindow::refreshNetworkUi);connect(wifiScan_,&QPushButton::clicked,this,[this]{NetworkResult r;const auto aps=networkService_->ScanWifi(wifiAdapter_->currentData().toString(),r);wifiNetworks_->setRowCount(0);for(const auto& ap:aps){const int row=wifiNetworks_->rowCount();wifiNetworks_->insertRow(row);wifiNetworks_->setItem(row,0,new QTableWidgetItem(ap.ssid));wifiNetworks_->setItem(row,1,new QTableWidgetItem(QString::number(ap.signal)+"%"));wifiNetworks_->setItem(row,2,new QTableWidgetItem(ap.secured?"Secured":"Open"));}wifiStatus_->setText(r.ok?QString("%1 networks available").arg(aps.size()):r.message);commandMessage_->setText(r.ok?"Wi-Fi scan complete.":"Wi-Fi scan failed.");});
    connect(wifiOn_,&QPushButton::clicked,this,[this]{const auto r=networkService_->SetWifiEnabled(true);wifiStatus_->setText(r.message);commandMessage_->setText(r.ok?"Wi-Fi enabled.":"Wi-Fi operation failed.");refreshNetworkUi();});connect(wifiOff_,&QPushButton::clicked,this,[this]{const auto r=networkService_->SetWifiEnabled(false);wifiStatus_->setText(r.message);commandMessage_->setText(r.ok?"Wi-Fi disabled.":"Wi-Fi operation failed.");refreshNetworkUi();});
    connect(wifiConnect_,&QPushButton::clicked,this,[this]{networkService_->SetEthercatAdapter(controller_->profile().adapterName);const int row=wifiNetworks_->currentRow();if(row<0){commandMessage_->setText("Select a Wi-Fi network.");return;}const QString ssid=wifiNetworks_->item(row,0)->text();const auto r=networkService_->ConnectWifi(wifiAdapter_->currentData().toString(),ssid,wifiPassword_->text());wifiPassword_->clear();wifiStatus_->setText(r.message);commandMessage_->setText(r.ok?"Wi-Fi connected.":"Wi-Fi connection failed.");refreshNetworkUi();});connect(wifiDisconnect_,&QPushButton::clicked,this,[this]{networkService_->SetEthercatAdapter(controller_->profile().adapterName);const auto r=networkService_->DisconnectWifi(wifiAdapter_->currentData().toString());wifiStatus_->setText(r.message);commandMessage_->setText(r.ok?"Wi-Fi disconnected.":"Wi-Fi disconnection failed.");refreshNetworkUi();});
    connect(wiredApply,&QPushButton::clicked,this,[this]{networkService_->SetEthercatAdapter(controller_->profile().adapterName);const QString device=wiredAdapter_->currentData().toString();NetworkResult r;if(wiredMode_->currentIndex()==0)r=networkService_->SetWiredDhcp(device);else r=networkService_->SetWiredStatic(device,wiredIp_->text(),wiredPrefix_->value(),wiredGateway_->text(),wiredDns_->text());wiredStatus_->setText(r.ok?"Configuration applied.":r.message);commandMessage_->setText(r.ok?"Network configuration applied.":"Network configuration failed.");});
    ...
    QTimer::singleShot(0,this,&MainWindow::refreshNetworkUi);
    return scrollPage(contents);
}

void MainWindow::refreshNetworkUi()
{
    QString protectedAdapter=controller_->profile().adapterName;if(adapterBox_&&!adapterBox_->currentData().toString().isEmpty())protectedAdapter=adapterBox_->currentData().toString();networkService_->SetEthercatAdapter(protectedAdapter);NetworkResult result;const auto adapters=networkService_->EnumerateAdapters(result);const QString oldWired=wiredAdapter_->currentData().toString(),oldWifi=wifiAdapter_->currentData().toString();wiredAdapter_->clear();wifiAdapter_->clear();bool wifiUsable=false,wifiConnected=false;
    for(const auto& a:adapters){const bool connected=a.state.contains("connected",Qt::CaseInsensitive)&&!a.state.contains("disconnected",Qt::CaseInsensitive);if(a.wifi){wifiAdapter_->addItem(a.device,a.device);wifiUsable=wifiUsable||!a.state.contains("unavailable",Qt::CaseInsensitive);wifiConnected=wifiConnected||connected;wifiStatus_->setText(QString("%1%2%3").arg(connected?"Connected":"Disconnected",a.connection.isEmpty()||a.connection=="--"?QString():" — "+a.connection,a.ipv4.isEmpty()?QString():" — IP "+a.ipv4));}else{wiredAdapter_->addItem(a.device,a.device);wiredStatus_->setText(QString("%1%2%3").arg(connected?"Connected":"Disconnected",a.connection.isEmpty()||a.connection=="--"?QString():" — "+a.connection,a.ipv4.isEmpty()?QString():" — IP "+a.ipv4));}}
    const int wiredIndex=wiredAdapter_->findData(oldWired);if(wiredIndex>=0)wiredAdapter_->setCurrentIndex(wiredIndex);const int wifiIndex=wifiAdapter_->findData(oldWifi);if(wifiIndex>=0)wifiAdapter_->setCurrentIndex(wifiIndex);
    const bool hasWifi=wifiAdapter_->count()>0;wifiOn_->setEnabled(hasWifi&&!wifiUsable);wifiOff_->setEnabled(hasWifi&&wifiUsable);wifiScan_->setEnabled(hasWifi&&wifiUsable);wifiConnect_->setEnabled(hasWifi&&wifiUsable&&!wifiConnected);wifiDisconnect_->setEnabled(hasWifi&&wifiConnected);wifiPassword_->setEnabled(hasWifi&&wifiUsable&&!wifiConnected);wifiNetworks_->setEnabled(hasWifi&&wifiUsable);
    if(!hasWifi)wifiStatus_->setText("No Wi-Fi adapter available.");if(wiredAdapter_->count()==0)wiredStatus_->setText(result.ok?"No Ethernet adapter available.":result.message);if(!result.ok)commandMessage_->setText("Network refresh failed.");
}

bool MainWindow::requestOrderlyExit(bool powerOff)
{
    if(shutdownCoordinator_->Busy())return false;exitButton_->setEnabled(false);commandMessage_->setText("Closing application...");QString error;
    if(!shutdownCoordinator_->Request(powerOff,error)){exitButton_->setEnabled(true);commandMessage_->setText(error);return false;}
    closeApproved_=true;QApplication::quit();return true;
}
...
QWidget* MainWindow::buildCalibration()
{
    ...
    auto* complianceBox=new QGroupBox("MACHINE COMPLIANCE COMPENSATION");auto* complianceLayout=new QVBoxLayout(complianceBox);complianceStatus_=new QLabel;complianceStatus_->setObjectName("ComplianceStatus");complianceEnabled_=new QCheckBox("Enabled");complianceEnabled_->setChecked(controller_->profile().complianceEnabled);complianceTable_=new QTableWidget(0,2);complianceTable_->setObjectName("CompliancePointTable");complianceTable_->setHorizontalHeaderLabels({"Force [N]","Deformation [mm]"});complianceTable_->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);for(const auto&p:controller_->profile().compliancePoints){const int row=complianceTable_->rowCount();complianceTable_->insertRow(row);complianceTable_->setItem(row,0,new QTableWidgetItem(QString::number(p.forceN,'g',17)));complianceTable_->setItem(row,1,new QTableWidgetItem(QString::number(p.deformationMm,'g',17)));}auto* complianceButtons=new QHBoxLayout;auto* captureZero=button("CAPTURE ZERO");auto* addPoint=button("ADD POINT");auto* deletePoint=button("DELETE POINT");auto* clearCurve=button("CLEAR CURVE");auto* applyCompliance=button("APPLY / SAVE COMPENSATION","primary");for(auto*b:{captureZero,addPoint,deletePoint,clearCurve,applyCompliance})complianceButtons->addWidget(b);complianceLayout->addWidget(complianceStatus_);complianceLayout->addWidget(complianceEnabled_);complianceLayout->addWidget(complianceTable_);complianceLayout->addLayout(complianceButtons);layout->addWidget(complianceBox,1,0,1,2);
    auto* autoBox=new QGroupBox("MACHINE COMPLIANCE AUTO CALIBRATION — SAFETY CONTROLLED");auto* af=new QFormLayout(autoBox);const auto& ac=controller_->profile().complianceAutoCalibration;autoCalibrationStatus_=new QLabel("IDLE — install a rigid reference fixture");autoCalibrationStatus_->setWordWrap(true);autoMaximumForce_=number(ac.maximumForceN,.001,1000000,3);autoForceStep_=number(ac.forceStepN,.001,1000000,3);autoApproachSpeed_=number(ac.approachSpeedMmPerMin,.001,10000,3);autoCalibrationSpeed_=number(ac.calibrationSpeedMmPerMin,.001,10000,3);autoFineSpeed_=number(ac.fineSpeedMmPerMin,.001,10000,3);autoReturnSpeed_=number(ac.returnSpeedMmPerMin,.001,10000,3);autoMaximumTravel_=number(ac.maximumTravelMm,.001,10000,3);autoPrecheckForce_=number(ac.precheckForceN,.001,1000000,3);autoPrecheckTravel_=number(ac.precheckMaximumTravelMm,.001,10000,3);autoTolerance_=number(ac.forceToleranceN,.001,1000000,3);autoStabilizationMs_=integer(ac.stabilizationTimeMs,1,60000);autoConfirmationTimeoutMs_=integer(ac.precheckConfirmationTimeoutMs,1000,600000);auto* startCompression=button("START COMPRESSION PRE-CHECK","warning");auto* startTension=button("START TENSION PRE-CHECK","warning");autoFullStart_=button("START FULL CALIBRATION","danger");autoAbort_=button("ABORT CALIBRATION","danger");autoSave_=button("SAVE CURVE");autoSaveEnable_=button("SAVE & ENABLE","primary");auto* discard=button("DISCARD");af->addRow(autoCalibrationStatus_);af->addRow("Load Cell Capacity [N] (Profile)",new QLabel(QString::number(controller_->profile().loadcellCapacityN,'f',3)));af->addRow("Maximum Calibration Force [N]",autoMaximumForce_);af->addRow("Force Step [N]",autoForceStep_);af->addRow("Approach Speed [mm/min]",autoApproachSpeed_);af->addRow("Calibration Speed [mm/min]",autoCalibrationSpeed_);af->addRow("Fine Speed [mm/min]",autoFineSpeed_);af->addRow("Return Speed [mm/min]",autoReturnSpeed_);af->addRow("Maximum Travel [mm]",autoMaximumTravel_);af->addRow("Pre-check Force [N]",autoPrecheckForce_);af->addRow("Pre-check Maximum Travel [mm]",autoPrecheckTravel_);af->addRow("Force Tolerance [N]",autoTolerance_);af->addRow("Stabilization [ms]",autoStabilizationMs_);af->addRow("Full-start Confirmation Timeout [ms]",autoConfirmationTimeoutMs_);auto* autoButtons=new QHBoxLayout;for(auto*b:{startCompression,startTension,autoFullStart_,autoAbort_,discard,autoSave_,autoSaveEnable_})autoButtons->addWidget(b);af->addRow(autoButtons);complianceLayout->insertWidget(1,autoBox);
    ...
    connect(captureZero,&QPushButton::clicked,controller_,&UtmUiController::captureComplianceZero);connect(addPoint,&QPushButton::clicked,this,[this]{if(!controller_->complianceZeroValid()){showCommandFailure("Add compliance point","CAPTURE ZERO is required");return;}const auto p=controller_->currentCompliancePoint();const int row=complianceTable_->rowCount();complianceTable_->insertRow(row);complianceTable_->setItem(row,0,new QTableWidgetItem(QString::number(p.forceN,'g',17)));complianceTable_->setItem(row,1,new QTableWidgetItem(QString::number(p.deformationMm,'g',17)));});connect(deletePoint,&QPushButton::clicked,this,[this]{const int row=complianceTable_->currentRow();if(row>=0)complianceTable_->removeRow(row);});connect(clearCurve,&QPushButton::clicked,this,[this]{complianceTable_->setRowCount(0);complianceEnabled_->setChecked(false);});connect(applyCompliance,&QPushButton::clicked,this,[this]{QVector<dao::utm::CompliancePoint> points;for(int row=0;row<complianceTable_->rowCount();++row){bool forceOk=false,deformationOk=false;const double force=complianceTable_->item(row,0)?complianceTable_->item(row,0)->text().toDouble(&forceOk):0;const double deformation=complianceTable_->item(row,1)?complianceTable_->item(row,1)->text().toDouble(&deformationOk):0;if(!forceOk||!deformationOk){showCommandFailure("Apply compliance","Point table contains an invalid number");return;}points.append({force,deformation});}QString error;if(!controller_->applyCompliance(points,complianceEnabled_->isChecked(),error))showCommandFailure("Apply compliance",error);});
    const auto startAuto=[this](int mode){auto c=controller_->profile().complianceAutoCalibration;c.mode=mode;c.motionDirection=mode==UTM_COMPLIANCE_MODE_COMPRESSION?UTM_DIRECTION_COMPRESSION:UTM_DIRECTION_TENSION;c.maximumForceN=autoMaximumForce_->value();c.forceStepN=autoForceStep_->value();c.approachSpeedMmPerMin=autoApproachSpeed_->value();c.calibrationSpeedMmPerMin=autoCalibrationSpeed_->value();c.fineSpeedMmPerMin=autoFineSpeed_->value();c.returnSpeedMmPerMin=autoReturnSpeed_->value();c.maximumTravelMm=autoMaximumTravel_->value();c.precheckForceN=autoPrecheckForce_->value();c.precheckMaximumTravelMm=autoPrecheckTravel_->value();c.forceToleranceN=autoTolerance_->value();c.stabilizationTimeMs=autoStabilizationMs_->value();c.precheckConfirmationTimeoutMs=autoConfirmationTimeoutMs_->value();QMessageBox box(QMessageBox::Warning,"MACHINE COMPLIANCE AUTO CALIBRATION",QString("Mode: %1\nLoad Cell Capacity: %2 N\nMaximum Force: %3 N\nMaximum Travel: %4 mm\n\nThe machine will move automatically and apply force to the rigid reference fixture.\n\nVerify the fixture, direction, load cell, jig, and clear moving area.").arg(mode==UTM_COMPLIANCE_MODE_COMPRESSION?"COMPRESSION":"TENSION").arg(controller_->profile().loadcellCapacityN).arg(c.maximumForceN).arg(c.maximumTravelMm),QMessageBox::Cancel|QMessageBox::Ok,this);box.button(QMessageBox::Ok)->setText("START PRE-CHECK");box.setDefaultButton(QMessageBox::Cancel);box.setEscapeButton(QMessageBox::Cancel);if(box.exec()!=QMessageBox::Ok)return;QString error;if(!controller_->startComplianceAutoCalibration(c,error))showCommandFailure("Auto calibration",error);};
    connect(startCompression,&QPushButton::clicked,this,[startAuto]{startAuto(UTM_COMPLIANCE_MODE_COMPRESSION);});connect(startTension,&QPushButton::clicked,this,[startAuto]{startAuto(UTM_COMPLIANCE_MODE_TENSION);});connect(autoFullStart_,&QPushButton::clicked,this,[this]{QString error;if(!controller_->confirmFullComplianceCalibration(error))showCommandFailure("Full calibration",error);});connect(autoAbort_,&QPushButton::clicked,controller_,&UtmUiController::abortComplianceAutoCalibration);connect(discard,&QPushButton::clicked,controller_,&UtmUiController::discardPendingComplianceCurve);connect(autoSave_,&QPushButton::clicked,this,[this]{QString error;if(!controller_->savePendingComplianceCurve(false,error))showCommandFailure("Save compliance curve",error);});connect(autoSaveEnable_,&QPushButton::clicked,this,[this]{QString error;if(!controller_->savePendingComplianceCurve(true,error))showCommandFailure("Save and enable compliance curve",error);});
...
void MainWindow::refresh()
{
    ...
    const auto& communication=controller_->communicationRuntime();
    machineState_->setText(base.machineState==UTM_MACHINE_STOPPED&&communication.motionInterrupted?"MOTION INTERRUPTED":machineStateName(base.machineState));
    ...
    ethercatState_->setText(communication.communicationUnstableWarning?"EtherCAT ⚠ UNSTABLE":QString("EtherCAT %1").arg(base.input.communicationValid?"● ONLINE":"✕ OFFLINE"));
    ...
    const auto compliance=controller_->complianceRuntime();complianceStatus_->setText(QString("Current Force %1 N  •  Raw %2 mm  •  Compensation %3 mm  •  Corrected %4 mm\nConfigured %5  •  Range %6  •  Curve v%7  •  Capture zero %8").arg(compliance.forceN,0,'f',6).arg(compliance.rawDisplacementMm,0,'f',6).arg(compliance.compensationMm,0,'f',6).arg(compliance.correctedDisplacementMm,0,'f',6).arg(compliance.configured?"YES":"NO").arg(compliance.inCalibrationRange?"IN":"OUT / CLAMPED").arg(compliance.curveVersion).arg(controller_->complianceZeroValid()?"VALID":"REQUIRED"));
    const auto& cc=controller_->complianceCurve(UTM_COMPLIANCE_MODE_COMPRESSION);const auto& tc=controller_->complianceCurve(UTM_COMPLIANCE_MODE_TENSION);complianceMainStatus_->setText(QString("COMPLIANCE C: %1 / T: %2").arg(cc.enabled?"ON":"OFF",tc.enabled?"ON":"OFF"));const auto ar=controller_->autoCalibrationRuntime();autoCalibrationStatus_->setText(QString("Mode %1   Stage %2\nTarget %3 N   Current %4 N\nReference %5 mm   Current %6 mm   Deformation %7 mm\nCaptured %8/%9   Travel %10 mm   Allowed max %11 N   Fault %12").arg(ar.mode==UTM_COMPLIANCE_MODE_COMPRESSION?"COMPRESSION":"TENSION",autoCalibrationStateName(ar.state)).arg(ar.targetForceN,0,'f',3).arg(ar.currentForceN,0,'f',3).arg(ar.referenceMachinePositionMm,0,'f',5).arg(ar.currentMachinePositionMm,0,'f',5).arg(ar.currentDeformationMm,0,'f',5).arg(ar.capturedPointCount).arg(ar.targetCount).arg(ar.travelUsedMm,0,'f',3).arg(ar.allowedMaximumForceN,0,'f',3).arg(ar.fault));autoFullStart_->setEnabled(ar.state==UTM_COMPLIANCE_CAL_WAITING_FULL_START);autoAbort_->setEnabled(ar.active);autoSave_->setEnabled(ar.pendingResult);autoSaveEnable_->setEnabled(ar.pendingResult);
    ...
    diagnosticText_->setText(diagnosticText_->text()+QString("\n\nCOMMUNICATION RECOVERY\n  State/stage/attempt: %1 / %2 / %3\n  Incident/active/interrupted: %4 / %5 / %6\n  Total/recovered/failed: %7 / %8 / %9\n  Rolling 1h/unstable: %10 / %11\n  Duration/max: %12 / %13 ms\n  WKC current/expected/min: %14 / %15 / %16\n  Bad/max: %17 / %18\n  Failed slave/state/AL: %19 / 0x%20 / 0x%21\n  Command/source, sequence/step, calibration: %22/%23, %24/%25, %26\n  Servo state/alignment: 0x%27 / %28")
        .arg(communication.communicationState).arg(communication.recoveryStage).arg(communication.recoveryAttempt)
        .arg(communication.incidentGeneration).arg(communication.incidentMotionActive).arg(communication.motionInterrupted)
        .arg(communication.totalIncidentCount).arg(communication.recoveredIncidentCount).arg(communication.recoveryFailureCount)
        .arg(communication.rollingHourIncidentCount).arg(communication.communicationUnstableWarning)
        .arg(communication.recoveryElapsedMs).arg(communication.maximumRecoveryDurationMs)
        .arg(communication.currentWkc).arg(communication.expectedWkc).arg(communication.minimumWkc)
        .arg(communication.consecutiveBadWkc).arg(communication.maximumConsecutiveBadWkc).arg(communication.failedSlaveIndex)
        .arg(communication.failedSlaveState,0,16).arg(communication.failedSlaveAlStatus,0,16)
        .arg(communication.incidentCommandType).arg(communication.incidentCommandSource)
        .arg(communication.incidentSequenceRunning).arg(communication.incidentSequenceStep).arg(communication.incidentCalibrationActive)
        .arg(communication.postRecoveryServoState,0,16).arg(communication.safeAlignmentResult));
...
{
    if(closeApproved_){event->accept();return;}
    event->ignore();requestOrderlyExit(false);
}
...
void MainWindow::applyStyle()
{
    qApp->setStyleSheet(R"(
        * { font-family: "Noto Sans", "DejaVu Sans"; font-size: 13px; }
        ...
        QStatusBar { background:#101820; border-top:1px solid #293a47; }
