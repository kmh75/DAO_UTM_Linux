...
bool DaoEngineCore::RequestDaoAdcSafeOp(
    int physicalSlaveIndex)
{
    // 엔진이 초기화되지 않았다면 상태 전환 금지
    if (!IsInitialized())
    {
        return false;
    }

    // EtherCAT 어댑터가 열리지 않았다면 상태 전환 금지
    if (!master_.IsOpen())
    {
        return false;
    }

    // 실제 SAFE-OP 전환은 Master 계층에서 수행
    ...
    DaoInternalProcessExchangeInfo& exchangeInfo)
{
    ...
    // EtherCAT 어댑터가 열리지 않은 상태에서는 통신 금지
    if (!master_.IsOpen())
    {
        exchangeInfo = {};
        exchangeInfo.physicalSlaveIndex =
    ...
    // 실제 1회 Process Data 왕복은 Master 계층에서 수행
    ...
    DaoInternalOperationalInfo& operationalInfo)
{
    ...
    // EtherCAT 어댑터가 열려 있지 않으면 OP 전환 금지
    if (!master_.IsOpen())
    {
        operationalInfo = {};
    ...
    DaoInternalAdcReadInfo& readInfo)
{
    ...
    // EtherCAT 어댑터가 열려 있지 않으면 ADC 읽기 금지
    if (!master_.IsOpen())
    {
        readInfo = {};
    ...
    // 실제 Process Data 왕복과 24바이트 복사는 Master에서 수행
    ...
    signed char mode)
{
    ...
    // SDO와 2ms PDO 통신이 동시에 같은 SOEM Context를
    // 사용하지 않도록 순환통신 중에는 모드 변경을 거부합니다.
